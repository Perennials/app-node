...
class Config {
	...
	*/
	...
	_getRel ( id, args, ctx, lastNodeCb ) {
		...
		if ( id.indexOf( '*' ) >= 0 ) {
			...
			for ( var key in ret ) {
				var val = ret[ key ];
				ret[ key ] = this._renderRel( val.Value, args, [ val.Context ] );
			}
		...
		for ( var i = 0, iend = ids.length - ( lastNodeCb ? 1 : 0 ); obj !== undefined && i < iend; ++i ) {
			var objid = ids[ i ];
			if ( objid == '_' ) {
				// skip	
			}
				...
				obj = obj[ objid ];
				if ( lastNodeCb && !Object.isObject( obj ) ) {
					break;
				}
				if ( obj !== undefined ) {
					ctx.push( obj );
		...
		if ( lastNodeCb ) {
			return lastNodeCb( ctx, ids, i );
		}

		return this._renderRel( obj, args, ctx.slice( 0, -1 ) );
	}
	...
	*/
	get ( id ) {
		...
		var ret = this._getRel( id, Array_slice.call( arguments, 1 ), [ this ] );
	...
	/**
	Sets a resource with given id.

	Id can be a string delimited with dots. If necessary, the whole chain of objects
	will be created in the first config up the chain that has any nodes of the tree
	that end with an object.

	@def function Config.set( id:String, value:mixed )
	@return mixed The value that has been set.
	*/
	set ( id, value ) {

		if ( this._setSoft( id, value ) ) {
			return value;
		}
		else {
			this._setHard( id, value );
		}
	...
	_setHard ( id, value ) {
		...
		function lastNodeCb ( ctx, ids, i ) {
			var end = ids.length - 1;
			var obj = ctx.last;
			while ( i < end ) {
				obj = (obj[ ids[ i++ ] ] = {});
			}
			return obj[ ids[ i ] ] = value;
		}

		return this._getRel( id, Array_slice.call( arguments, 1 ), [ this ], lastNodeCb );
	}
	...
	_setSoft ( id, value ) {
		...
		if ( this._getRel( id, Array_slice.call( arguments, 1 ), [ this ], lastNodeCb ) ) {
			return true;
		}

		if ( this.Parent ) {
			return this.Parent._setSoft( id, value );
		}

		return false;
	}

	_renderRel ( text, args, ctx ) {
			...
			return text.replace( RE_VARIABLE, function ( match, id ) {
				...
				// no number or invalid number, it must be a property ref then
					...
					if ( id.startsWith( '_.' ) || id.startsWith( '__.' ) ) {
						value = _this._getRel( id, null, [].concat( ctx ) );
					}
					else {
						value = _this.get( id );
					}
	...
	*/
	render ( text ) {
		return this._renderRel( text, Array_slice.call( arguments, 1 ), [ this ] );
	}
}
